...
export default function KanbanBoard() {
  ...
  useSocket(); // realtime sync

  useEffect(() => {
    dispatch(fetchTasks())
      .unwrap()
      .then(() => setBackendReady(true)) // ✅ first successful response
...
const handleAdd = (title, column) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      ...
      <div className="mb-4 flex justify-center gap-3">
        {!backendReady ? (
          <div className="bg-yellow-200 text-yellow-800 px-4 py-2 rounded">
            ⏳ Backend is starting... please wait (Render cold start ~3–4 mins)
          </div>
        ) : (
          <button
