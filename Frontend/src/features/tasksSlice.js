...
export const addTask = createAsyncThunk(
  ...
  async (task, { rejectWithValue }) => {
    try {
      const r = await axios.post(`${API}/add`, task);
      if (BROADCAST) await axios.post(BROADCAST); // notify others
      return r.data;
    } catch (err) {
      return rejectWithValue(task);
    }
...
export const updateTask = createAsyncThunk(
  ...
  async ({ id, updates }, { rejectWithValue }) => {
    try {
      const r = await axios.put(`${API}/update/${id}`, updates);
      if (BROADCAST) await axios.post(BROADCAST);
      return r.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
    }
...
const tasksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // sync backend with optimistic
      .addCase(addTask.fulfilled, (s, a) => {
        // replace fake task with real one
        const idx = s.items.findIndex((t) => t._id === a.meta.arg._id);
        if (idx !== -1) {
